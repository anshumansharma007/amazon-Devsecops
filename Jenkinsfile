pipeline {
    ...
    stages {
        ...
        stage("Git Checkout") {
            steps {
                git branch: 'main', url: 'https://github.com/anshumansharma007/amazon-Devsecops.git'
            }
        }
        ...
        stage("Build Docker Image") {
            steps {
                script {
                    env.IMAGE_TAG = "anshuman8765/amazon:${BUILD_NUMBER}"
            ...
            steps {
                script {
                    withCredentials([string(credentialsId: 'docker-cred', variable: 'dockerpwd')]) {
                        sh "docker login -u anshuman8765 -p ${dockerpwd}"
                        sh "docker tag amazon ${env.IMAGE_TAG}"
                        sh "docker push ${env.IMAGE_TAG}"

                        // Also push latest
                        sh "docker tag amazon anshuman8765/amazon:latest"
                        sh "docker push anshuman8765/amazon:latest"
                    }
                }
            }
        }
    ...
    always {
        script {
            ...
            emailext (
                ...
                """,
                to: 'anshumansharma007@gmail.com',
                from: 'anshumansharma007@gmail.com',
                mimeType: 'text/html',
